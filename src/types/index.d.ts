...
export interface CreditsConfig {
  ...
  // Daily login reward config - designed to build habit, then convert to paid
  dailyLoginCredits?: {
    enable: boolean;                 // Whether to enable daily login credits
    amount: number;                  // Credits per login (should equal WAV2LIP_CREDITS_PER_TASK for zero-frustration UX)
    expireDays: number;              // Days until expiry (1 = forces daily use, prevents hoarding)
    maxLifetimeClaims: number;       // Max total claims ever (e.g. 5 = 5-day free trial window, then paywall)
  };
...
export type MenuItem = {
  ...
  align?: 'top' | 'bottom';           // The vertical alignment of the item (for sidebars)
};
