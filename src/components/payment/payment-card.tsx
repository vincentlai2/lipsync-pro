...
import { trackPurchase } from '@/lib/ads-tracking';
...
export function PaymentCard() {
  ...
  useEffect(() => {
    if (paymentCheck?.isPaid && status === 'processing') {
      ...
      // Trigger conversion tracking once
      if (!hasFiredPurchaseRef.current) {
        hasFiredPurchaseRef.current = true;
        console.log('[PaymentCard] Triggering purchase conversion tracking');
        trackPurchase({
          transactionId: sessionId || '',
          value: (paymentCheck.amount || 0) / 100, // convert cents to major currency unit
          currency: paymentCheck.currency || 'EUR',
          planId: paymentCheck.planId || '',
          priceId: paymentCheck.priceId || '',
        });
      }
    }
  }, [paymentCheck, status, sessionId]);
