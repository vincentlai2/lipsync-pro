...
const checkoutSchema = z.object({
  ...
  cancelCallbackUrl: z.string().optional(),
});
...
export const createCheckoutAction = userActionClient
  ...
  .action(async ({ parsedInput, ctx }) => {
    const { planId, priceId, metadata, successCallbackUrl, cancelCallbackUrl } =
      parsedInput;
    ...
    try {
      ...
      const successUrl = getUrlWithLocale(
        `${Routes.Payment}?session_id={CHECKOUT_SESSION_ID}&callback=${successCallbackUrl || Routes.SettingsBilling}`,
        locale
      );
      const cancelUrl = getUrlWithLocale(
        cancelCallbackUrl || Routes.SettingsBilling,
        locale
      );
